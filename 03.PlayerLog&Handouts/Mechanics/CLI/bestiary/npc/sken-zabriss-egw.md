---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- ttrpg-cli/compendium/src/5e/egw
- ttrpg-cli/monster/cr/1
- ttrpg-cli/monster/size/medium
- ttrpg-cli/monster/type/humanoid/goliath
statblock: inline
statblock-link: "#^statblock"
aliases:
- Sken Zabriss
---
# [Sken Zabriss](03.PlayerLog&Handouts\Mechanics\CLI\bestiary\npc/sken-zabriss-egw.md)
*Source: Explorer's Guide to Wildemount p. 221*  

```statblock
"name": "Sken Zabriss (EGW)"
"size": "Medium"
"type": "humanoid"
"subtype": "goliath"
"alignment": "Lawful Evil"
"ac": !!int "16"
"ac_class": "[breastplate](03.PlayerLog&Handouts/Mechanics/CLI/items/breastplate.md),\
  \ [shield](03.PlayerLog&Handouts/Mechanics/CLI/items/shield.md)"
"hp": !!int "45"
"hit_dice": "7d8 + 14"
"modifier": !!int "0"
"stats":
  - !!int "16"
  - !!int "11"
  - !!int "15"
  - !!int "13"
  - !!int "10"
  - !!int "12"
"speed": "30 ft."
"senses": "passive Perception 10"
"languages": "Common, Draconic, Giant"
"cr": "1"
"traits":
  - "desc": "Sken counts as one size larger when determining her carrying capacity\
      \ and the weight she can push, drag, or lift."
    "name": "Powerful Build"
  - "desc": "Sken wears a [ring of obscuring](03.PlayerLog&Handouts/Mechanics/CLI/items/ring-of-obscuring-egw.md).\
      \ With it, she can cast the [fog cloud](03.PlayerLog&Handouts/Mechanics/CLI/spells/fog-cloud.md)\
      \ spell centered on herself three times per day. The cloud lasts for 1 minute\
      \ (no [concentration](03.PlayerLog&Handouts/Mechanics/CLI/rules/conditions.md#Concentration)\
      \ required)."
    "name": "Special Equipment"
"actions":
  - "desc": "Melee Weapon Attack: +5 to hit, reach 5 ft., one target. Hit: 7\
      \ (1d8 + 3) slashing damage, or 8 (1d10 + 3) slashing damage when used with\
      \ two hands."
    "name": "Longsword"
"reactions":
  - "desc": "When Sken takes damage, she can use her reaction to reduce the damage\
      \ taken by 8 (1d12 + 2)."
    "name": "Stone's Endurance (Recharges after a Short or Long Rest)"
"source":
  - "EGW"
"image": "03.PlayerLog&Handouts/Mechanics/CLI/bestiary/npc/token/sken-zabriss-egw.webp"
```
^statblock